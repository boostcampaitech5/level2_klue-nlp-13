...
def mask_tokens(tokenizer, input_ids:torch.Tensor, mlm_prob:float=0.15, do_rep_random:bool=True):
    ...
    entity_ids = [tokenizer.convert_tokens_to_ids('<subject_entity>'), tokenizer.convert_tokens_to_ids('<object_entity>')]
    ...
    if do_rep_random:
        ...
        random_words = torch.randint(len(tokenizer)-len(entity_ids), labels.shape, dtype=torch.long)
...
def load_tokenizer(args):
    ...
    new_tokens=['<subject_entity>', '<object_entity>']
